...
# System configuration (space-separated pairs for active-active; single-node uses 0)
%{ if node_count > 1 ~}
...
# Object cold tier (objbacker): S3 buckets tenant datasets replicate onto from the
# local-NVMe hot pool. Empty unless terraform ran with bucket_count > 0. Auth uses the
# EC2 instance role -> access key is the role name, secret is empty (setup-cold-pool.sh
# treats an empty secret on aws as "use the instance role").
...
export MAYASCALE_S3_SECRET_KEY="${s3_secret_key}"
